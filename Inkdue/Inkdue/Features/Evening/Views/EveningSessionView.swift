...
struct EveningSessionView: View {
    @Environment(\.dismiss) private var dismiss
    @StateObject private var viewModel: EveningSessionViewModel
    var onSessionCompleted: (() -> Void)?

    init(
        viewModel: EveningSessionViewModel,
    ...
    private func cardSection(_ card: EveningSessionCard) -> some View {
    ...
    private var completionSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Complete evening to advance study day.")
                .font(.subheadline)
                .foregroundStyle(.secondary)

            Button("Complete Evening") {
    ...
    private var emptySection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Evening queue is empty.")
                .font(.headline)
            Text("Complete this phase to move to next study day.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
        }
        .padding(14)
    ...
    private var errorSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Could not load evening data.")
                .font(.headline)
            Button("Retry") {
...
#Preview {
    NavigationStack {
        EveningSessionView(
            viewModel: EveningSessionViewModel(repository: AppContainer.live.repository)
        )
    }
}
