...
final class EveningSessionViewModel: ObservableObject {
    struct State: Equatable {
        ...
        var currentCard: EveningSessionCard? {
    ...
    private func loadQueue(isInitialLoad: Bool) {
        ...
        do {
            ...
            let queueItems = try BuildEveningQueueUseCase(repository: repository)
            ...
            let cards = queueItems.compactMap { item -> EveningSessionCard? in
                guard let word = wordMap[item.wordId], let srs = srsMap[item.wordId] else {
                    return nil
                }

                return EveningSessionCard(
                    wordId: item.wordId,
                    term: word.term,
    ...
    private func completeSession() {
        ...
        do {
            let appState = try repository.fetchOrCreateAppState()
            guard appState.currentPhase == .evening else {
                state.errorMessage = "Current phase is not evening."
                return
            }

            let transition = AdvancePhaseUseCase().execute(on: appState)
            try repository.save()
            state.currentStudyDayIndex = transition.nextStudyDayIndex
